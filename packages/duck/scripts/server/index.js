...
const appStarted = (startedPort, startedHost) => {
  ...
  logger.log(divider);
  logger.log(`Localhost: ${chalk.magenta(`http://${startedHost}:${startedPort}${config.publicPath}`)}`);
  logger.log(`LAN: ${chalk.magenta(`http://${ip.address()}:${startedPort}${config.publicPath}`)}`);
  logger.log(divider);
